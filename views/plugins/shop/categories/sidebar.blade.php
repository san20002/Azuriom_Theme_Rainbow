    ...
    @foreach($categories as $subCategory)
        ...
        @foreach($subCategory->categories as $cat)
            <a href="{{ route('shop.categories.show', $cat) }}"
               class="list-group-item pl-5 @if($category->is($cat)) active @endif">
                {{ $cat->name }}
            </a>
        @endforeach
    @endforeach
...
@auth
    ...
    <div class="mb-4">
        ...
        <a href="{{ route('shop.cart.index') }}"
           class="btn btn-primary btn-block">{{ trans('shop::messages.cart.title') }}</a>
    </div>
@endauth
    ...
    <div class="mb-4">
        ...
        <div class="progress mb-1">
            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                 aria-valuenow="{{ $goal }}" aria-valuemin="0" aria-valuemax="100" style="width: {{ $goal }}%"></div>
        </div>
